...
func (r *planRepo) Find(ctx context.Context, id string) (calendar.Plan, error) {
	r.m.RLock()
	defer r.m.RUnlock()
	...
	return calendar.Plan{}, cerror.NewNotFoundError(
		nil,
		fmt.Sprintf("not found plan(%v)", id),
	)
}

func (r *planRepo) Create(ctx context.Context, plan calendar.Plan) error {
	r.m.RLock()
...
func (r *planRepo) Update(ctx context.Context, plan calendar.Plan) error {
	r.m.Lock()
	defer r.m.Unlock()
