...
func (r *sessionRepo) Find(ctx context.Context, id string) (auth.Session, error) {
	r.m.RLock()
	defer r.m.RUnlock()
	...
	return auth.Session{}, cerror.NewNotFoundError(
		nil,
		fmt.Sprintf("not found session(%v)", id),
	)
}

func (r *sessionRepo) FindByUserID(ctx context.Context, userID string) (auth.Session, error) {
	r.m.RLock()
	defer r.m.RUnlock()
